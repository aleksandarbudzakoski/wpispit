...
public class ProductServiceImpl implements ProductService {
    ...
    public Product create(String name, Double price, Integer quantity, List<Long> categories) {
        List<Category> categoriesNames = this.categoryRepository.findAllById(categories);

        Product product = new Product(name,price,quantity,categoriesNames);
    ...
    public Product update(Long id, String name, Double price, Integer quantity, List<Long> categories) {
        ...
        List<Category> categoriesNames = this.categoryRepository.findAllById(categories);
        product.setCategories(categoriesNames);
...
//                .filter(i->i.getName().contains(name) && i.getCategories().stream().anyMatch(j->j.getId().equals(categoryId)))
